class Game extends React.Component {
  ...
  initGame() {
    let game = Object.assign({}, this.state.juego);
    this.setState({
      game: {
        ...game,
        game_state: 1
  ...
  nextPlayer(player) {
    return (player + 1) % this.state.juego.players.length;
  }
  ...
  componentDidUpdate() {
    // Set up bot play
    if (isBotsTurn(this.state.juego)) {
      botPlay(this.state.juego, this.state.board);
    }
  }
  ...
  render() {
    ...
    if (this.state.juego.game_state === 1) {
      game.push(
        <DraggableAssignBox
          key="assignmentbox"
          game={this.state.juego}
